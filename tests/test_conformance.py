...
class SDKV3ConformanceTests(unittest.TestCase):
    ...
    def test_allocation_and_typed_value_contracts(self) -> None:
        self.assertEqual(self.fixture["version"], 6)
    ...
    def test_required_features_use_modules_and_module_phases_are_ordered(self) -> None:
        order = []
        modules = []
        for name in ("first", "second"):
            modules.append({
                "name": name,
                "before": lambda options, name=name: order.append(f"before:{name}") or options,
                "beforeEvaluation": lambda options, name=name: order.append(f"beforeEvaluation:{name}") or options,
                "afterEvaluation": lambda evaluation, _options, name=name: order.append(f"afterEvaluation:{name}") or evaluation,
                "after": lambda evaluation, _options, name=name: order.append(f"after:{name}") or evaluation,
            })
        f = create_featurevisor({
            "datafile": self.fixture["requiredFeatures"]["datafile"],
            "modules": modules,
            "logLevel": "fatal",
        })
        self.assertTrue(f.is_enabled("enabledFeature"))
        self.assertEqual(order, self.fixture["modulePipeline"]["featureOrder"])

        f = create_featurevisor({
            "datafile": self.fixture["requiredFeatures"]["datafile"],
            "modules": [{
                "name": "redirect-required-feature",
                "beforeEvaluation": lambda options: {
                    **options,
                    "featureKey": "disabledFeature" if options.get("featureKey") == "enabledFeature" else options.get("featureKey"),
                },
            }],
            "logLevel": "fatal",
        })
        self.assertFalse(f.is_enabled("stringRequirement"))
